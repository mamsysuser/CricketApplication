...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('global.match.title_singular') }}
    </div>
        ...
        <form action="{{ route("admin.matches.update", [$match->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('firstteam_id') ? 'has-error' : '' }}">
                ...
                <select class="form-control" name="firstteam_id" id="firstteam_id">
                  ...
                  @foreach ($teams as $key => $value)
                    <option value="{{ $key }}"
                    {{ $match->firstteam_id == $key ? 'selected' : '' }}
                    > 
                        {{ $value }} 
                    </option>
                  @endforeach    
            ...
            <div class="form-group {{ $errors->has('secondteam_id') ? 'has-error' : '' }}">
                ...
                <select class="form-control" name="secondteam_id" id="secondteam_id">
                  ...
                  @foreach ($teams as $key => $value)
                    <option value="{{ $key }}"
                    {{ $match->secondteam_id == $key ? 'selected' : '' }}
                    > 
                        {{ $value }} 
                    </option>
                  @endforeach    
            ...
            <div class="form-group {{ $errors->has('winningteam_id') ? 'has-error' : '' }}">
                ...
                <select class="form-control" name="winningteam_id" id="winningteam_id">
                  ...
                  @foreach ($teams as $key => $value)
                    <option value="{{ $key }}"
                    {{ $match->winningteam_id == $key ? 'selected' : '' }}
                    > 
                        {{ $value }} 
                    </option>
                  @endforeach    
